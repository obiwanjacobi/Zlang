...
llvm::Type* LlvmIrBuilder::getType(const AstType* type)
{
...
void LlvmIrBuilder::VisitFunction(AstFunction* function) {
    ...
    {
        params.push_back(getType(p->getTypeReference().get()));
    }

    auto retRefType = function->getTypeReference().get();
    auto retType = retRefType ? getType(retRefType) : Builder.getVoidTy();
